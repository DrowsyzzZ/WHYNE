...
export function SignupPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      if (result.session) void navigate('/', { replace: true });
      else
        void navigate('/login', {
          replace: true,
          state: { notice: '이메일 확인 후 로그인해주세요.' },
        });
    } catch (error) {
      const detail = error instanceof Error ? error.message : '';
      const message = /already|registered/i.test(detail)
        ? '이미 가입된 이메일입니다.'
        : `회원가입에 실패했습니다.${detail ? ` (${detail})` : ' 잠시 후 다시 시도해주세요.'}`;
