...
const initialFilters: WineFilterValues = { search: '', types: [], minPrice: 0, maxPrice: 500000, ratingMin: null, ratingMax: null, likedOnly: false };
const PAGE_SIZE = 8;
...
export function WineListPage() {
  ...
  const filteredWines = filters.likedOnly ? data.filter((wine) => likedWineIdSet.has(wine.id)) : data;
  ...
  const updateFilters = (next: WineFilterValues) => {
    if (next.likedOnly && !user) { setShowLikeLogin(true); return; }
    setFilters(next);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries[0]?.isIntersecting) setVisibleCount((count) => Math.min(count + PAGE_SIZE, filteredWines.length));
  ...
  const handleToggleLike = (wineId: string) => {
    if (!user) { setShowLikeLogin(true); return; }
  ...
  return (
    <main className="bg-white pb-24">
      <section className="bg-gray-100 py-10 tablet:py-14">
        <div className="container-whyne"><h1 className="text-xl font-bold tablet:text-2xl">이번 달 추천 와인</h1><RecommendedWineCarousel likedWineIds={likedWineIdSet} onOpen={(id) => void navigate(`/wines/${id}`)} onToggleLike={handleToggleLike} wines={recommendedWines} /></div>
      </section>
      ...
      <div className="mt-8 grid gap-10 desktop:grid-cols-[260px_1fr]">
        ...
        <section aria-busy={isFetching} aria-label="와인 검색 결과">
          {isLoading ? <Loading label="와인 목록을 불러오는 중" /> : error ? <ErrorState onRetry={() => void refetch()} /> : visibleWines.length === 0 ? <EmptyState description={filters.likedOnly ? '좋아요한 와인이 없습니다.' : filters.search ? `'${filters.search}'에 대한 검색 결과가 없습니다.` : '선택한 조건에 맞는 와인이 없습니다.'} title={filters.likedOnly ? '좋아하는 와인을 모아보세요' : '와인을 찾지 못했어요'} /> : <><div className="grid grid-cols-1 gap-x-8 gap-y-16 tablet:grid-cols-2">{visibleWines.map((wine) => <WineCard key={wine.id} onOpen={(id) => void navigate(`/wines/${id}`)} onToggleLike={handleToggleLike} wine={{ ...wine, isLiked: likedWineIdSet.has(wine.id) }} />)}</div>{hasMore && <div aria-label="다음 와인 불러오기" className="h-10" ref={loadMoreRef} />}</>}
        </section>
      </div>
      </section>
      ...
      <Modal isOpen={showLikeLogin} onClose={() => setShowLikeLogin(false)} size="sm" title="로그인이 필요해요"><p className="text-gray-600">와인을 좋아요 목록에 저장하려면 로그인해주세요.</p><div className="mt-6 flex justify-end gap-3"><Button onClick={() => setShowLikeLogin(false)} variant="secondary">취소</Button><Button onClick={() => void navigate('/login')}>로그인</Button></div></Modal>
