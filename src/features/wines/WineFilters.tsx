...
export function WineFilters({ filters, onChange }: { filters: WineFilterValues; onChange: (next: WineFilterValues) => void }) {
  ...
  return (
    <aside aria-label="와인 필터" className="space-y-12">
      <fieldset><legend className="mb-5 text-xl font-bold">타입</legend><div className="grid justify-items-start gap-3">{typeOptions.map((option) => { const selected = selectedType === option.value; return <button aria-pressed={selected} className={`flex min-h-12 items-center gap-3 rounded-full border px-3 pr-5 text-lg transition-colors ${selected ? 'border-primary bg-primary text-gray-100' : 'border-gray-300 bg-white text-gray-900'}`} key={option.value} onClick={() => toggleType(option.value)} type="button"><img alt="" className="size-8 rounded-full object-cover" src={option.image} />{option.label}</button>; })}</div></fieldset>
